...
h1 {text-align:center;}
    ...
    <tbody>
      ...
      <div class = "selection">
        ...
        <h1>Top Fantasy Running Backs</h1>
    
    <?php
    
                                          
    //create an array to hold the rb fantasy points    
    $rbpArray = array();
    
    $index = 0; //to help print the arrays to the table
                                            
    //create array to hold matching rb names                                        
    $rbnArray = array();
    
    //array used to find the next largest stat in the table
    $checkArray = array();
   ...
   if ($conn->connect_error){
    ...
    if($records->num_rows > 0){ 
       ...
       while($row = $records->fetch_assoc()){
            ...
            //math is done to determine the amount of fantasy points each rusher has
           $points = ($yards * .1) + ($touchdowns * 6);
        ...
        for($i = 0; $i < count($rbpArray); $i++){
            for($j = 0; $j < (count($rbpArray) - $i -1); $j++){
                if($rbpArray[$j] < $rbpArray[$j+1])
                {
                    //switch the points around if one is bigger than the other
                    $temp1 = $rbpArray[$j];
                    $rbpArray[$j] = $rbpArray[$j+1];
                    $rbpArray[$j+1] = $temp1;
                    
                    //have to switch the names too
                    $temp2 = $rbnArray[$j];
                    $rbnArray[$j] = $rbnArray[$j+1];
                    $rbnArray[$j+1] = $temp2;
                }
            }//end of nested for loop
        }//end of for loop
        //output the rest of the table
        while($index != count($rbpArray)){       
        echo "<tr><td>" .($index+1) ."</td><td>" .$rbnArray[$index] ."</td><td>" .$rbpArray[$index] ."</td></tr>";
        $index++;
        }
      echo '</table>';
    } 
