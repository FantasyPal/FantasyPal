...
//#include all the header files up here when youre ready
using namespace std;

void teamCheck(const string teamname, ifstream &filein);					//remove Team names from all vector containers
...
bool sortPassingAttempts(Passing *a, Passing *b);							//used for std::sort on stat values for Passer Container
...
int main()
{
	...
	vector<Passing *> passerContainer;				//create a vector to hold pointers to a Passing object
	...
	do{
		...
		if (filein.is_open())
		{
			...
			do{
				...
				getline(filein, garbage, '\'');						//getline up to '     *for some reason apostrophe is '\'' lol* 
				filein >> attempts;
				getline(filein, garbage, '\n');
				getline(filein, garbage, '\'');
				filein >> completions;
				...
				if (filein.peek() == ','){			//sometimes there's a comma for QB's who don't ride the bench
					filein >> comma;
